...
{
    public partial class MsgWall : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    strSiteCode = Common.Common.NoHtml(Request["SiteCode"].ToString());
                }
                else
                {
                    return;
        ...
        void ShowMsgList()
        {
            ...
            if (null != ChatMsgDs && ChatMsgDs.Tables.Count > 0 && ChatMsgDs.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    nickname=ChatMsgDs.Tables[0].Rows[i]["NickName"].ToString();
                    string openid = ChatMsgDs.Tables[0].Rows[i]["OpenID"].ToString();
                    if (nickname == null || nickname == "")
                    {
                        if (openid != null && openid != "")
                        {
                            nickname = wxuserdal.GetUserValueByOpenID("NickName", openid).ToString();
                            userheadimg = "\r\n<img src=\""+
                                wxuserdal.GetUserValueByOpenID("HeadImgUrl", openid).ToString() + "\">";
                        }
                        else
                        {
                    ...
                    "\r\n<tbody>" +
                        "\r\n<tr>" +
                            "\r\n<td class=\"tdFace\" rowspan=\"2\">" +
                                "\r\n<a  class=\"aUserInfo\"><span class=\"pName\">" + nickname + "</span>" +userheadimg+
                                    "</a>" +
                            "\r\n</td>" +
                            "\r\n<td class=\"tdChat\">" +
                                "\r\n<div class=\"article\">" +
        ...
        void getLotteryUserList()
        {
            ...
            if (null != ChatUserDs && ChatUserDs.Tables.Count > 0 && ChatUserDs.Tables[0].Rows.Count > 0)
            {
                ...
                msglist += "\r\n</span>";
                msglist += "\r\n<span id=\"rockname\" data-id=\"\"  data-rmid=\"\" class=\"rock-name\"></span>";
                ...
                for (int i = 0; i < ChatUserDs.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        nickname = ChatUserDs.Tables[0].Rows[i]["NickName"].ToString();
                    }

                    script += "{ \"id\": \"" + UserID + "\",\"rmid\":\"" + strRoomID + "\","+
                        "\"nickname\":\"" + nickname + "\"}";
        ...
        void SaveWin()
        {
            ...
            if (uid.Trim() != null && uid.Trim() != "" &&rmid.Trim() != null && rmid.Trim() != "")
            {
        ...
        void UpdateIsWin()
        {
            if (strRoomID.Trim() != null && strRoomID.Trim() != "")
            {
                if (userdal.UpdateAllIsWinByRoomID(strRoomID))
                {
                    Response.Write("{\"success\":\"true\"}");
                }
                else
