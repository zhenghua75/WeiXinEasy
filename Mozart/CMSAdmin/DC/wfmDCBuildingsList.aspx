<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmDCBuildingsList.aspx.cs" Inherits="Mozart.CMSAdmin.DC.wfmDCBuildingsList" %>
...
<head id="Head1" runat="server">
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            id: 'wfmDCBuildingsAdd',
	            title: '添加楼盘信息',
	            top: '20px',
	            width: '1000px',
	            height: '600px',
	            content: 'url:DC/wfmDCBuildingsAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            id: 'wfmDCBuildingsUpdate',
	            title: '楼盘信息修改',
	            top: '20px',
	            width: '1000px',
	            height: '600px',
	            content: 'url:DC/wfmDCBuildingsUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:DC/wfmDCBuildingsUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加楼盘信息" onclick="AddFunction()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC">  
						...
						<td><%#Eval("Name") %></td>
                        <td><%#Eval("AVEPrice") %></td>
                        <td><%#Eval("GreenRate") %></td>
                        <td><%#Eval("VolumeRate") %></td>
                        <td><%#Eval("PropertyDevelopers") %></td>
                        <td><%#Eval("PropertyCompany") %></td>
                        <td><%#Eval("Regions") %></td>
                        <td><%#Eval("CreateTime") %></td>
						<td>
                            <a href="#" onclick="ShowArticle('<%#Eval("ID") %>');">详细内容</a>&nbsp;|&nbsp;
                            <a href="#" onclick="UpdateArticle('<%#Eval("ID") %>');">修改</a>&nbsp;|&nbsp;
                            <a href="#" onclick="AdminBizControlConfirm('wfmDCBuildingsAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a>
						</td>
					</tr>
			</ItemTemplate>
			<FooterTemplate>
