var imageUrl = '/CMSAdmin/TemplateManage/Images/color.png';
            ...
            for (k = 0; k < tblCells.length; k++) {
                jQuery(tblChilds[j].childNodes[k]).unbind().mouseover(function (a) {
                    ...
                    if (id == 'txtTextColor') {
                        ...
                        $('#func_table img').attr('textcolor', aaa);
                        doc.find('p').css('color', aaa);
                    }
                    else if (id == 'txtBgColor') {
                        $('#menu_textcolor span').removeClass('active');
                        $('#menu_table img').attr('textcolor', aaa);
                    }
                    else if (id == 'txtBg2Color') {
                        $('.bgColors span').removeClass('active');
                        $('#modifyIcon .imgs > img.active').attr('backgroundcolor', aaa);
                    }
                    else if (id == 'txtWzColor') {
                        $('.wzColors span').removeClass('active');
                        $('#modifyIcon .imgs > img.active').attr('textcolor', aaa);
                    } else if (id == 'txtQjsColor') {
                        var ecid = Request.GetQuery('ecid');

                        var bg = $('#txtBjsColor').attr('color') ? $('#txtBjsColor').attr('color') : 'ffffff';
                        var fg = aaa.substr(1);

                        $('#txtQjsColor').attr('color', fg);
                        var erweima = 'http://qr.liantu.com/api.php?&bg=' + bg + '&fg=' + fg + '&w=128&el=h&text=http://ci.f3.cn/d/' + ecid;
                        $('#erweima').attr('src', erweima);
                        $('#erweima_xiazai').attr('href', erweima);
                    } else if (id == 'txtBjsColor') {
                        var ecid = Request.GetQuery('ecid');

                        var bg = aaa.substr(1);
                        var fg = $('#txtQjsColor').attr('color') ? $('#txtQjsColor').attr('color') : '000000';

                        $('#txtBjsColor').attr('color', fg);
                        var erweima = 'http://qr.liantu.com/api.php?&bg=' + bg + '&fg=' + fg + '&w=128&el=h&text=http://ci.f3.cn/d/' + ecid;
                        $('#erweima').attr('src', erweima);
                        $('#erweima_xiazai').attr('href', erweima);
                    }
...
this.iColorPicker = function () {
    jQuery("input.iColorPicker").each(function (i) {
        ...
        jQuery(this).css("backgroundColor", jQuery(this).val()).after('<a href="javascript:void(null)" id="icp_' + this.id + '" onclick="iColorShow(\'' + this.id + '\',\'icp_' + this.id + '\');"><img src="' + imageUrl + '" style="border:0;margin:7px 0 0 4px" align="absmiddle" ></a>')
    })
};
jQuery(function () {
    iColorPicker()
