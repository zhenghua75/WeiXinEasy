...
jQuery.validator.addMethod("positiveIntegerContainZero", function (value, element) { return this.optional(element) || /^(0|[1-9]\d*)$/.test(value); }, "只允许输入正整数或零");
...
jQuery.validator.addMethod("Mobile", function (value, element) { var length = value.length; return this.optional(element) || (length == 11 && /^1[0-9]{8,20}$/.test(value)); }, "请正确填写您的手机号码");
 ...
 * 验证正数  大于0的数字 最多两位小数
 */
$.validator.addMethod('numberOrdigits2', function (value, element) {
    var reg = /^[0-9]+([.]\d{1,2})?$/;
    if (!reg.test(value)) {
        return false;
    } else {
 ...
 * 大于
 */
$.validator.addMethod('compareInt_more', function (value, element) {
    // var param=$(element).metadata().id;
    var param = $(element).attr("compare");
    var curVal = parseInt(value);
    //var targetVal=parseInt($(param).val());
 ...
 * 小于
 */
$.validator.addMethod('compareInt_less', function (value, element) {
    var param = $(element).attr("compare");
    var curVal = parseInt(value);
    var targetVal = $("#" + param).val();
    if (targetVal == 0) {
 ...
 * 验证脚本
 */
$.validator.addMethod("isScript", function (value, element) {
    //var reg = /^(\<script\s[\^\\%&\*~'\?\/\<\>\|\"`]*\<\/script\>)+$/;
    //return reg.test(value);
    value = $.trim(value.toLowerCase());
    return ((value.indexOf('<script') > -1) && (value.indexOf('<script') == 0)) & (value.indexOf('</script>') > -1 && (value.lastIndexOf('</script>') == (value.length - 9)));
}, "请填入正确脚本");
