<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmChatMsg.aspx.cs" Inherits="Mozart.CMSAdmin.CR.wfmChatMsg" %>
		...
		<asp:Panel ID="Panel1" runat="server" Height="31px" HorizontalAlign="Center">
			<table>
				<tr>
					<td>消息内容：</td>
					<td>
						<asp:TextBox ID="MsgText" runat="server" Width="80px"></asp:TextBox>
					</td>
                    <td>聊天室：</td>
					<td>
                    ...
                    <td>消息状态：</td>
					<td>
						<asp:DropDownList ID="ddlmsgstate"  runat="server">
                    ...
                    <td><asp:Button Text="未通过" id="nopassbtn" runat="server" OnClick="nopassbtn_Click" /></td>
                    <td>
                        <asp:Button Text="通过" ID="passbtn" runat="server" OnClick="passbtn_Click" /></td>
				</tr>
			</table>
		</asp:Panel>
	</div>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC"> 
                        <td><asp:Label ID="no"  runat="server" Text='<%#Eval("ID") %>'></asp:Label>
                            <asp:Label ID="msgstate" Text='<%#Eval("MsgState") %>' Visible="false"  runat="server" />
                            <asp:Label ID="msgtype" Text='<%#Eval("MsgType") %>' Visible="false"  runat="server" />
                        </td>  
                        <td><%#Eval("NickName") %></td>                 
						<td><%#Eval("RoomName") %></td>
                        <td>
                            <asp:Label ID="msgtext" Text='<%#Eval("MsgText") %>'  runat="server" />
                        </td>
                        <td><%#Eval("AddTime") %></td>
						<td>
                            <ul>
                                <li><asp:Label ID="state"  runat="server" /></li>
                                <li>&nbsp;&nbsp;&nbsp;<a href="#" onclick="AdminBizControlConfirm('wfmChatMsgAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
