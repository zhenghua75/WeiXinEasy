<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmChatRoomList.aspx.cs" Inherits="Mozart.CMSAdmin.CR.wfmChatRoomList" %>
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:CR/wfmChatRoomAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:CR/wfmChatRoomUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:CR/wfmChatRoomUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加聊天室" onclick="AddFunction()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC"> 
                        ...
                        <td><%#Eval("PhoneNum") %></td>
                        <td>
                            <asp:Label ID="roomimg" Text='<%#Eval("RoomImg") %>' runat="server" />
                        </td>
                        <td><%#Eval("AddTime") %></td>
						<td>
                            <ul>
                                <li><a href="#" onclick="ShowArticle('<%#Eval("ID") %>');">详细内容</a>&nbsp;|&nbsp;</li>
                                <li><a target="_blank" href='../../WXWall/MsgWall.aspx?SiteCode=<%=Session["strSiteCode"] %>&rmid=<%#Eval("ID") %>'>查看大屏幕</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="UpdateArticle('<%#Eval("ID") %>');">修改</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="AdminBizControlConfirm('wfmChatRoomAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
