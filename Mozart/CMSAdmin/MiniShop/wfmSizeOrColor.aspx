<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmSizeOrColor.aspx.cs" EnableEventValidation="false" Inherits="Mozart.CMSAdmin.MiniShop.wfmSizeOrColor" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
	<title>产品颜色、尺寸管理</title>
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmSizeOrColorAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmSizeOrColorUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmSizeOrColorUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
		...
		<asp:Panel ID="Panel1" runat="server" Height="31px" HorizontalAlign="Center">
			<table>
				<tr>
                    ...
                    <td>选择颜色或尺寸：</td>
                    <td>
                        <asp:DropDownList id="sizeorcolor" runat="server">
                            <asp:ListItem Text="全部" Value="" /><asp:ListItem Text="没有" Value="0" />
                            <asp:ListItem Text="颜色" Value="-1" /><asp:ListItem Text="尺寸" Value="1" />
                        </asp:DropDownList>
                    </td>
                    <td>
                        <asp:Button ID="btnQuery" runat="server" Text="查询" OnClick="btnQuery_Click" />&nbsp;
                        <input type="button" value="添加产品属性" onclick="AddFunction()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC"> 
                        ...
                        <td><%#Eval("shopname")%></td>
                        <td><%#Eval("Ptitle")%></td>
                        <td><%#Eval("SizeColor")%></td>
                        <td><%#Eval("ScName")%></td>
                        <td><asp:Label ID="scimg" Text='<%#Eval("Scimg") %>' runat="server" /></td>
                        <td><%#Eval("Stock")%></td>
                        <td><%#Eval("AddTime") %></td>
						<td>
                            <ul>
                                ...
                                <li><a href="#" onclick="AdminBizControlConfirm('wfmSizeOrColorAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
    ...
    <script>
        ...
        function GetSortOption(id) {
            ...
            if (value == 0 || value == "0") { }
                ...
                $.getJSON("wfmSizeOrColor.aspx", { "action": "getcateoption", "subid": value }, function (json) {
        ...
        function GetProductOption(id, subid) {
            ...
            if (value == 0 || value == "0") { }
                ...
                $.getJSON("wfmSizeOrColor.aspx", { "action": "getpoption", "subid": subvalue, "catid": value }, function (json) {
                    $.each(json, function (i, item) {
                        var optionID = item.ID;
