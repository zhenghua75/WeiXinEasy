<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmProductAtlas.aspx.cs" EnableEventValidation="false" Inherits="Mozart.CMSAdmin.MiniShop.wfmProductAtlas" %>
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmProductAtlasAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmProductAtlasUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmProductAtlasUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加图集" onclick="AddFunction()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC"> 
						...
						<td><%#Eval("AtlasName") %></td>
                        <td><asp:Label ID="atlas" Text='<%#Eval("PimgUrl") %>' runat="server" /></td>
                        <td><%#Eval("ShopName")%></td>
                        <td><%#Eval("Ptitle")%></td>
						...
						<td>
                            <ul>
                                ...
                                <li><a href="#" onclick="AdminBizControlConfirm('wfmProductAtlasAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
    ...
    <script>
        ...
        function GetSortOption(id) {
            ...
            if (value == 0 || value == "0") { }
                ...
                $.getJSON("wfmProductAtlas.aspx", { "action": "getcateoption", "subid": value }, function (json) {
        ...
        function GetProductOption(id,subid) {
            ...
            if (value == 0 || value == "0") { }
                ...
                $.getJSON("wfmProductAtlas.aspx", { "action": "getpoption", "subid": subvalue,"catid":value }, function (json) {
                    $.each(json, function (i, item) {
                        var optionID = item.ID;
