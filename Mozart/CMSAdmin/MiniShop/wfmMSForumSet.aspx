<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmMSForumSet.aspx.cs" Inherits="Mozart.CMSAdmin.MiniShop.wfmMSForumSet" %>
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            ...
	            width: '800px',
	            height: '400px',
	            content: 'url:MiniShop/wfmMSForumSetAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '800px',
	            height: '400px',
	            content: 'url:MiniShop/wfmMSForumSetUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '800px',
	            height: '400px',
	            content: 'url:MiniShop/wfmMSForumSetUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC"> 
                        ...
                        <td><%#Eval("FTitle")%></td>
                        <td><asp:Label ID="logoimg" Text='<%#Eval("LogoImg") %>' runat="server" /></td>
                        <td><asp:Label ID="backimg" Text='<%#Eval("BackImg") %>' runat="server" /></td>
                        <td><%#Eval("Visit")%></td><td><%#Eval("topiccount")%></td>
                        <td><%#Eval("AddTime") %></td>
						<td>
                            <ul>
                                <li><a href="#" onclick="ShowArticle('<%#Eval("ID") %>');">详细内容</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="UpdateArticle('<%#Eval("ID") %>');">修改</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="AdminBizControlConfirm('wfmMSForumSetAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
