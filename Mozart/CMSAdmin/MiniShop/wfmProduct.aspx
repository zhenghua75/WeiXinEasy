<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmProduct.aspx.cs" Inherits="Mozart.CMSAdmin.MiniShop.wfmProduct" %>
	...
	<script type="text/javascript">
	    ...
	    function ShowArticle(strid,cid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:MiniShop/wfmProductUpdate.aspx?action=show&id=' + strid + "&cid=" + cid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC"> 
                        ...
                        <td><%#Eval("Ptitle")%></td>
                        <td>￥<%#Eval("Price") %>元</td>
                        <td><%#Eval("IsSecHand")%></td>
                        <td><%#Eval("Review").ToString()=="0"?"未审核":"已审核"%></td>
                        <td><%#Eval("AddTime") %></td>
						<td>
                            <ul>
                                <li><a href="#" onclick="ShowArticle('<%#Eval("ID") %>','<%#Eval("CID") %>');">详细内容</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="UpdateArticle('<%#Eval("ID") %>','<%#Eval("CID") %>');">修改</a>&nbsp;|&nbsp;</li>
                                <li><asp:Label ID="labelbtn" Text='<%#Eval("Review") %>'
                                     ToolTip='<%#Eval("ID") %>' runat="server" />&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="AdminBizControlConfirm('wfmProductAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
