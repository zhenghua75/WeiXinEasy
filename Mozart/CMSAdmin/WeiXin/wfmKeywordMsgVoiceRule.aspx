<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmKeywordMsgVoiceRule.aspx.cs" Inherits="Mozart.CMSAdmin.WeiXin.wfmKeywordMsgVoiceRule" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
	<title>关键字语音回复管理</title>
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            ...
	            width: '520px',
	            height: '280px',
	            content: 'url:WeiXin/wfmKeywordMsgVoiceRuleEdit.aspx?action=add',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '460px',
	            height: '245px',
	            content: 'url:WeiXin/wfmKeywordMsgVoiceRuleEdit.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '460px',
	            height: '245px',
	            content: 'url:WeiXin/wfmKeywordMsgVoiceRuleEdit.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function VoicePlay(strid) {
	        $.dialog({
	            ...
	            width: '460px',
	            height: '245px',
	            content: 'url:WeiXin/wfmKeywordMsgVoicePlay.aspx?id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加关键字语音回复" onclick="AddFunction()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC">  
						...
						<td><%#Eval("MatchPattern") %></td>
                        <td><%#Eval("WXName") %></td>
                        <td><asp:Label Text='<%#Eval("MatchType") %>' ID="MatchType" runat="server" /></td>
                        <td><asp:Label Text='<%#Eval("MsgType") %>' ID="msgtype" runat="server" /> </td>
                        <td><%#Eval("AddTime") %></td><td><%#Eval("Order") %></td>
						<td>
                            <a href="#" onclick="VoicePlay('<%#Eval("MsgValue") %>');">播放</a>&nbsp;|&nbsp;
                            <a href="#" onclick="AdminBizControlConfirm('wfmKeywordMsgVoiceRuleEdit.aspx?action=del&id=<%#Eval("ID") %>');">删除</a>
						</td>
					</tr>
			</ItemTemplate>
			<FooterTemplate>
