<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmJCQuizList.aspx.cs" Inherits="Mozart.CMSAdmin.JC.wfmJCQuizList" %>
	...
	<script type="text/javascript">
	    function AddFunction() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:JC/wfmJCQuizAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:JC/wfmJCQuizUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:JC/wfmJCQuizUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function Showlosewin(strid)
	    {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:JC/wfmJCQuizWin.aspx?action=show&quizID=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
		...
		<asp:Panel ID="Panel1" runat="server" Height="31px" HorizontalAlign="Center">
			<table>
				<tr>
                    ...
                    <td>开始时间：</td>
					<td>
						<asp:TextBox ID="starttime" runat="server" Width="80px"></asp:TextBox>
					</td>
                    <td>
                        <asp:Button ID="btnQuery" runat="server" Text="查询" OnClick="btnQuery_Click" />&nbsp;
                        <input type="button" value="添加竞猜信息" onclick="AddFunction()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC">  
						...
						<td>
                            <ul>
                                <li><asp:Label ID="ShowLoseWin" Text='<%#Eval("ID") %>' ToolTip='<%#Eval("RightScore") %>' runat="server" /></li>
                                <li><a href="#" onclick="ShowArticle('<%#Eval("ID") %>');">详细内容</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="UpdateArticle('<%#Eval("ID") %>');">修改</a>&nbsp;|&nbsp;</li>
                                <li><a href="#" onclick="AdminBizControlConfirm('wfmJCQuizAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a></li>
                            </ul>
						</td>
					</tr>
			</ItemTemplate>
