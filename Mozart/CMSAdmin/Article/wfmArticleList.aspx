<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmArticleList.aspx.cs" Inherits="Mozart.CMSAdmin.Article.wfmArticleList" %>
	...
	<script type="text/javascript">
	    function AddArticle() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '640px',
	            content: 'url:Article/wfmArticleAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '640px',
	            content: 'url:Article/wfmArticleUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '640px',
	            content: 'url:Article/wfmArticleUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加文章" onclick="AddArticle()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC">  
						...
						<td><img src="../../<%#DataBinder.Eval(Container.DataItem, "Pic")%>" alt="图标" width="20px",height="20px"/></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "CategoryName")%></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "ClickNum")%></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "CreateTime")%></td>
                        <td><%#DataBinder.Eval(Container.DataItem, "Order")%></td>
						<td>
                            <a href="#" onclick="ShowArticle('<%#DataBinder.Eval(Container.DataItem, "ID")%>');">详细内容</a>&nbsp;|&nbsp;
                            <a href="#" onclick="UpdateArticle('<%#DataBinder.Eval(Container.DataItem, "ID")%>');">修改</a>&nbsp;|&nbsp;
                            <a href="#" onclick="AdminBizControlConfirm('wfmArticleAdmin.aspx?action=del&id=<%#DataBinder.Eval(Container.DataItem, "ID")%>');">删除</a>
						</td>
					</tr>
			</ItemTemplate>
			<FooterTemplate>
