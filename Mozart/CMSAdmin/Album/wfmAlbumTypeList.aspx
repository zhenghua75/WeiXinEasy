<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmAlbumTypeList.aspx.cs" Inherits="Mozart.CMSAdmin.Album.wfmAlbumTypeList" %>
	...
	<script type="text/javascript">
	    function AddArticle() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:Album/wfmAlbumTypeAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            id: 'wfmAlbumTypeUpdate',
	            title: '相册修改',
	            top: '20px',
	            width: '1000px',
	            height: '600px',
	            content: 'url:Album/wfmAlbumTypeUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:Album/wfmAlbumTypeUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加类别" onclick="AddArticle()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound">
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC">  
						...
						<td><%#Eval("Name") %></td>
                        <td><img src="../../<%#Eval("cover")%>" alt='<%#Eval("Name") %>' width="20px" height="20px"/></td>
                        <td> <asp:Label ID="thestate" runat="server" /> </td>
						<td>
                            <a href="#" onclick="ShowArticle('<%#Eval("ID") %>');">详细内容</a>&nbsp;|&nbsp;
                            <a href="#" onclick="UpdateArticle('<%#Eval("ID") %>');">修改</a>&nbsp;|&nbsp;
                            <a href="#" onclick="AdminBizControlConfirm('wfmAlbumTypeAdmin.aspx?action=del&id=<%#Eval("ID") %>');">
