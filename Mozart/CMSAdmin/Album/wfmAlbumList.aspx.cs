...
{
    public partial class wfmAlbumList : System.Web.UI.Page
    {
        ...
        {
            string strWhere = " b.SiteCode = '" + Session["strSiteCode"].ToString() + "' ";
            if (!string.IsNullOrEmpty(this.ddlCategory.SelectedValue) && ddlCategory.SelectedValue.Trim()!="0")
            {
                strWhere = strWhere + " AND b.ID = '" + ddlCategory.SelectedValue + "' ";
            }
            if (!string.IsNullOrEmpty(this.txtTitle.Text))
            {
                strWhere = strWhere + " AND a.Name LIKE '%" + this.txtTitle.Text + "%' ";
            }
            AspNetPager1.CurrentPageIndex = 1;
