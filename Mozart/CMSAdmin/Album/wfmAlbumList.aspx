<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfmAlbumList.aspx.cs" Inherits="Mozart.CMSAdmin.Album.wfmAlbumList" %>
	...
	<script type="text/javascript">
	    function AddArticle() {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:Album/wfmAlbumAdd.aspx',
	            lock: true,
	            close: function () {
	    ...
	    function UpdateArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:Album/wfmAlbumUpdate.aspx?action=update&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
	    ...
	    function ShowArticle(strid) {
	        $.dialog({
	            ...
	            width: '1000px',
	            height: '600px',
	            content: 'url:Album/wfmAlbumUpdate.aspx?action=show&id=' + strid,
	            lock: true,
	            close: function () {
	                __doPostBack('btnQuery', '');
	            }
			...
			<table>
				<tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="添加相册" onclick="AddArticle()" />
                    </td>
				</tr>
			</table>
		</asp:Panel>
		...
		<asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" >
			...
			<ItemTemplate>
					<tr style="background-color:#FAF3DC">  
						...
						<td><%#Eval("Name") %></td>
                        <td><%#Eval("typename") %></td>
                        <td><img src="../../<%#Eval("Photo")%>" alt='<%#Eval("Name") %>' width="20px" height="20px"/></td>
						<td>
                            <a href="#" onclick="ShowArticle('<%#Eval("ID") %>');">详细内容</a>&nbsp;|&nbsp;
                            <a href="#" onclick="UpdateArticle('<%#Eval("ID") %>');">修改</a>&nbsp;|&nbsp;
                            <a href="#" onclick="AdminBizControlConfirm('wfmAlbumAdmin.aspx?action=del&id=<%#Eval("ID") %>');">删除</a>
						</td>
					</tr>
			</ItemTemplate>
			<FooterTemplate>
